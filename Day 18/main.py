...
async def websocket_audio_streaming(websocket: WebSocket):
    ...
    def on_turn(self: Type[StreamingClient], event: TurnEvent):
        ...
        if event.end_of_turn:
            ...
            print(f"\nEND OF TURN - FINAL TRANSCRIPTION: {transcript_text}")

            # --- STREAMING LLM RESPONSE ---
            try:
                for chunk in llm.stream_response(transcript_text):
                    print(chunk, end="", flush=True)  # Prints LLM response live
                print("\n--- LLM RESPONSE COMPLETE ---\n")
            except Exception as e:
                logging.error(f"LLM streaming failed: {e}")

            # Send end-of-turn transcription to client
            try:
