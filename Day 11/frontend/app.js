// app.js - Frontend logic for Day 9 + Day 10 AI Voice Agent + Day 11 Error Handling
...
if (data.error) {
  const fallbackText = data.llmResponse || "I'm having trouble connecting right now.";
  ...
  if (data.audioUrl) {
    ...
    try {
      ...
      // If audio playback blocked, fallback to speech synthesis
      console.warn("Fallback audio playback failed, using speech synthesis.", e);
      speakFallback(fallbackText);
    }
  } else {
    // No fallback audio URL, use speech synthesis directly
    speakFallback(fallbackText);
  }

  setStatus("Error from server — fallback response used", "#ef4444");
