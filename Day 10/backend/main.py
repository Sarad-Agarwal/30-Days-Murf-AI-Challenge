...
# ----------- Chat History Store (Global Dictionary) -----------
...
# ----------- DAY 10 Endpoint (Chat with History) -----------
...
async def agent_chat(session_id: str, file: UploadFile = File(...)):
    try:
        ...
        # Step 3: Prepare prompt with history
        conversation_text = "\n".join(
        ...
        # Step 4: Send to LLM
        model = genai.GenerativeModel("gemini-2.5-flash")
        llm_response = model.generate_content(conversation_text).text

        # Step 5: Store assistant reply
        chat_history[session_id].append({"role": "assistant", "content": llm_response})

        # Step 6: Convert assistant reply to speech
        murf_audio_url = generate_murf_audio(llm_response)
        ...
        return {
            ...
            "llmResponse": llm_response,
            "audioUrl": murf_audio_url,
            "chatHistory": chat_history[session_id]
        }
    except Exception as e:
        return {"error": str(e)}
